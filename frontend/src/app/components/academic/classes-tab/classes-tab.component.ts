...
import { ClassSummary, CreateClassRequest, UpdateClassRequest } from '../../../models/academic.models';
...
export class ClassesTabComponent implements OnInit, OnDestroy {
  ...
  loadClasses(): void {
    ...
    this.classService.listClasses({ search: this.searchTerm() || undefined })
  ...
  openAddDialog(): void {
    ...
    this.classForm.reset({ name: '', sortOrder: null });
    this.showDialog.set(true);
  }

  openEditDialog(classItem: ClassSummary): void {
    this.editClassId.set(classItem.id);
    this.classForm.reset({ name: classItem.name, sortOrder: classItem.sortOrder ?? null });
    this.showDialog.set(true);
  }
